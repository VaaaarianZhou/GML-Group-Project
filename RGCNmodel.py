...
# Define the Heterogeneous RGCN mode
class HeteroRGCN(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels):
        ...
        self.conv2 = HeteroConv({
            ('cell', 'spatially_close_to', 'cell'): GCNConv(hidden_channels, hidden_channels),
            ('cell', 'similar_to', 'cell'): GCNConv(hidden_channels, hidden_channels),
        }, aggr='mean')
...
for dist_threshold in dist_thresholds:
    ...
    train_X, train_y, labeled_spatial_edges, labeled_similarity_edges, inverse_dict = load_hubmap_data(train_df,
                                                                                                       dist_threshold,
    ...
    test_loader = HGTLoader(train_hetero_data, num_samples=[1024] * 2, batch_size=128,
