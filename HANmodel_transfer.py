...
from utils import load_hubmap_data, visualize_predictions, write_result_to_csv, EarlyStopping
import pandas as pd
...
for dist_threshold in dist_thresholds:
    ...
    train_X, train_y, labeled_spatial_edges, labeled_similarity_edges, inverse_dict = load_hubmap_data(train_df, dist_threshold, neighborhood_size_threshold, sample_rate)
    ...
    train_loader = HGTLoader(train_hetero_data, num_samples=[1024]*2, shuffle=True, batch_size = 128, input_nodes=('cell', train_hetero_data['cell'].train_mask))
    val_loader = HGTLoader(train_hetero_data, num_samples=[1024]*2, batch_size = 128, input_nodes=('cell', train_hetero_data['cell'].val_mask))
    test_loader = HGTLoader(train_hetero_data, num_samples=[1024]*2, batch_size = 128, input_nodes=('cell', train_hetero_data['cell'].test_mask))
    ...
    torch.save(model.state_dict(), os.path.join(MODEL_PATH, f'HAN_model_{dist_threshold}_{neighborhood_size_threshold}_weights.pth'))
